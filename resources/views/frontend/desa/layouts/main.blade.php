...
<body class="bg-gray-50 dark:bg-gray-900">
    ...
    <header class="bg-white dark:bg-gray-800 shadow">
            ...
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <button data-theme-toggle
                        class="text-gray-500 hover:text-gray-700 dark:hover:text-gray-200 dark:text-gray-400 transition-colors">
                        ...
                        <svg class="dark:hidden block w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <!-- Navigation -->
        <nav class="bg-primary-600 dark:bg-gray-700">
            <div class="mx-auto px-4 container">
                <div class="flex justify-between items-center h-16">
                    ...
                    <!-- Mobile menu button -->
                    <div class="md:hidden">
                        <button data-mobile-menu-toggle class="text-white hover:text-gray-200 transition-colors">
        ...
        <div class="mx-auto px-4 container">
            <div class="gap-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
                ...
                <div>
                    ...
                    <p class="text-gray-300">Alamat: {{ $desa->alamat ?? '-' }}</p>
                    <p class="text-gray-300">Email: {{ $desa->email ?? '-' }}</p>
                    <p class="text-gray-300">Telepon: {{ $desa->telepon ?? '-' }}</p>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-2">
                        <li><a href="{{ route('desa.berita', $desa->uri) }}"
                                class="text-gray-300 hover:text-white">Berita</a></li>
                        <li><a href="{{ route('desa.layanan-publik', $desa->uri) }}"
                                class="text-gray-300 hover:text-white">Layanan Publik</a></li>
                        <li><a href="{{ route('desa.publikasi', $desa->uri) }}"
                                class="text-gray-300 hover:text-white">Publikasi</a></li>
                        <li><a href="{{ route('desa.pengaduan', $desa->uri) }}"
                                class="text-gray-300 hover:text-white">Pengaduan</a></li>
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <div class="flex space-x-4">
                        @if ($desa->facebook)
                            <a href="{{ $desa->facebook }}" target="_blank" class="text-gray-300 hover:text-white">
                        ...
                        @if ($desa->twitter)
                            <a href="{{ $desa->twitter }}" target="_blank" class="text-gray-300 hover:text-white">
                                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path
                        ...
                        @if ($desa->instagram)
                            <a href="{{ $desa->instagram }}" target="_blank" class="text-gray-300 hover:text-white">
                                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path fill-rule="evenodd"
                        ...
                        @if ($desa->youtube)
                            <a href="{{ $desa->youtube }}" target="_blank" class="text-gray-300 hover:text-white">
                                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path fill-rule="evenodd"
            ...
            <div class="mt-8 pt-8 border-gray-700 border-t text-gray-400 text-center">
    ...
    <div class="right-4 bottom-4 z-50 fixed space-y-3">
        ...
        <div class="relative">
            ...
            <div data-visitor-stats-modal
                class="hidden right-0 bottom-full absolute bg-white dark:bg-gray-800 shadow-lg mb-2 p-4 rounded-lg w-64">
                <h3 class="mb-3 font-semibold text-gray-900 dark:text-white">Statistik Pengunjung</h3>
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        <span class="text-gray-600 dark:text-gray-400">Hari Ini:</span>
                        <span class="font-medium" id="stats-today">-</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600 dark:text-gray-400">Kemarin:</span>
                        <span class="font-medium" id="stats-yesterday">-</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600 dark:text-gray-400">Minggu Ini:</span>
                        <span class="font-medium" id="stats-week">-</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600 dark:text-gray-400">Bulan Ini:</span>
                        <span class="font-medium" id="stats-month">-</span>
                    </div>
                    <hr class="my-2">
                    <div class="flex justify-between">
                        <span class="text-gray-600 dark:text-gray-400">Total:</span>
                        <span class="font-bold text-blue-600" id="stats-total">-</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- Theme Toggle Button -->
        <button data-theme-toggle
            class="bg-gray-700 dark:bg-gray-200 shadow-lg p-3 rounded-full text-white dark:text-gray-800 transition-colors">
    ...
    <div class="bottom-4 left-4 z-50 fixed">
        <a href="{{ route('desa.pengaduan', $desa->uri) }}"
            class="flex justify-center items-center bg-yellow-500 hover:bg-yellow-600 shadow-lg p-3 rounded-full text-white transition-colors">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    @if (session('success') || session('error'))
        <div
            class="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-50 {{ session('error') ? 'bg-red-500' : 'bg-green-500' }} text-white p-4 rounded-lg shadow-lg max-w-xs">
